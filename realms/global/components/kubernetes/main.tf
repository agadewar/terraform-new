...
resource "azurerm_kubernetes_cluster" "kubernetes" {
  ...
  # This is the old SP. Need to replace it with new SP, but requires a rebuild of Kubernetes.
  service_principal {
    client_id     = var.old_service_principal_app_id
    client_secret = var.old_service_principal_password
  }
...
data "template_file" "autoscaler_config" {
  ...
  vars = {
    autoscaler_client_id           = base64encode(var.old_service_principal_app_id)
    autoscaler_client_secret       = base64encode(var.old_service_principal_password)
    ...
    autoscaler_tenant_id           = base64encode(var.old_service_principal_tenant)

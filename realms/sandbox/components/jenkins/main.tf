...
resource "kubernetes_secret" "maven_repo_azure_file" {
  ...
  data = {
    azurestorageaccountname = data.terraform_remote_state.storage_account.outputs.storage_account_name
    azurestorageaccountkey  = data.terraform_remote_state.storage_account.outputs.storage_account_access_key
  }
}

resource "kubernetes_persistent_volume_claim" "maven_repo" {
  depends_on = [
    null_resource.maven_repo_storage_class,
    null_resource.maven_repo_pv,
  ]
...
data "template_file" "maven_repo_pv" {
  ...
  vars = {
    ...
    share_name          = data.terraform_remote_state.storage_account.outputs.storage_account_name
  }
}
...
resource "azurerm_network_interface" "jenkins_windows_slave_nic" {
  depends_on = [
    azurerm_public_ip.jenkins_windows_slave,
    azurerm_network_security_group.jenkins_windows_slave,
  ]

...
locals {
  ...
  node_pool_profile_2_name    = "query"
  node_pool_profile_3_name    = "application"
  node_pool_profile_4_name    = "build"
...
resource "azurerm_kubernetes_cluster" "kubernetes" {
  ...
  default_node_pool {
    ...
    vm_size              = var.kubernetes_system_vm_size
    os_disk_size_gb      = var.kubernetes_system_os_disk_size_gb
    enable_auto_scaling  = true
    min_count            = var.kubernetes_system_min_count
    max_count            = var.kubernetes_system_max_count
    max_pods             = 250
...
resource "azurerm_kubernetes_cluster_node_pool" "pool02" {
  ...
  vm_size               = var.kubernetes_query_vm_size
  os_type               = var.kubernetes_query_os_type
  os_disk_size_gb       = var.kubernetes_query_os_disk_size_gb
  enable_auto_scaling   = true
  min_count             = var.kubernetes_query_min_count
  max_count             = var.kubernetes_query_max_count
  max_pods              = 250
...
resource "azurerm_kubernetes_cluster_node_pool" "pool03" {
  ...
  kubernetes_cluster_id = azurerm_kubernetes_cluster.kubernetes.id
  vm_size               = var.kubernetes_application_vm_size
  os_type               = var.kubernetes_application_os_type
  os_disk_size_gb       = var.kubernetes_application_os_disk_size_gb
  enable_auto_scaling   = true
  min_count             = var.kubernetes_application_min_count
  max_count             = var.kubernetes_application_max_count
  max_pods              = 250
  vnet_subnet_id        = data.terraform_remote_state.network.outputs.aks-pool_subnet_id #ALL NODES MUST BELONG TO THE SAME SUBNET
}
...
resource "azurerm_kubernetes_cluster_node_pool" "pool04" {
  ...
  kubernetes_cluster_id = azurerm_kubernetes_cluster.kubernetes.id
  vm_size               = var.kubernetes_build_vm_size
  os_type               = var.kubernetes_build_os_type
  os_disk_size_gb       = var.kubernetes_build_os_disk_size_gb
  enable_auto_scaling   = true
  min_count             = var.kubernetes_build_min_count
  max_count             = var.kubernetes_build_max_count
  max_pods              = 250
  vnet_subnet_id        = data.terraform_remote_state.network.outputs.aks-pool_subnet_id #ALL NODES MUST BELONG TO THE SAME SUBNET
}

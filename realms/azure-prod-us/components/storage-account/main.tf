...
resource "azurerm_storage_account" "talend_storage_account_prod" {
  name                      = "talend${replace(var.realm, "-", "")}prod"
  resource_group_name       = var.resource_group_name
  location                  = var.resource_group_location
  account_tier              = "Standard"
  account_kind              = "StorageV2"
...
resource "azurerm_storage_container" "talend_storage_container_prod" {
  name                  = "talend-${replace(var.realm, "-", "")}-prod"
  resource_group_name   = var.resource_group_name
  storage_account_name  = azurerm_storage_account.talend_storage_account_prod.name
  container_access_type = "blob"
}
...
resource "null_resource" "azure_data_lake_storage_gen2" {
  ...
  provisioner "local-exec" {
    ...
    command = "az storage account create --name sapdl${replace(lower(var.realm), "-", "")}${substr(replace(lower(var.environment), "-", ""), 0, 14)} --subscription ${var.subscription_id} --resource-group ${var.resource_group_name} --location ${var.resource_group_location} --sku Standard_LRS --kind StorageV2 --hierarchical-namespace true"
  }

  provisioner "local-exec" {
    when = "destroy"

    command = "az storage account delete --name sapdl${replace(lower(var.realm), "-", "")}${substr(replace(lower(var.environment), "-", ""), 0, 14)} --subscription ${var.subscription_id} --resource-group ${var.resource_group_name} --yes"
  }
}
...
resource "null_resource" "azure_data_lake_storage_gen2_key_1" {
  ...
  provisioner "local-exec" {
    command = "mkdir -p .local && az storage account keys list --account-name sapdl${replace(lower(var.realm), "-", "")}${substr(replace(lower(var.environment), "-", ""), 0, 14)} --subscription ${var.subscription_id} --resource-group ${var.resource_group_name} | jq -r .[0].value > .local/azure_data_lake_storage_gen2.key1"
  }
}
...
resource "null_resource" "azure_data_lake_storage_gen2_key_2" {
  ...
  provisioner "local-exec" {
    command = "mkdir -p .local && az storage account keys list --account-name sapdl${replace(lower(var.realm), "-", "")}${substr(replace(lower(var.environment), "-", ""), 0, 14)} --subscription ${var.subscription_id} --resource-group ${var.resource_group_name} | jq -r .[1].value > .local/azure_data_lake_storage_gen2.key2"
  }
}

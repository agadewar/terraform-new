...
locals {
  ...
  common_tags = "${merge(
    ...
    map(
      "Component", "Consul Storage"
    )
  )}"
}

data "template_file" "consul_home_pv" {
  template = "${file("templates/consul-home-pv.yaml.tpl")}"
...
resource "azurerm_managed_disk" "consul_home" {
  name                 = "consul-home"
...
resource "kubernetes_persistent_volume_claim" "consul_home" {
  depends_on = [ "null_resource.consul_home_pv","kubernetes_namespace.namespace" ]
  ...
  spec {
    ...
    volume_name = "consul-home"  // This is what it should be when PV is created under TF: "${null_resource.sonarqube.metadata.0.name}"
    storage_class_name = "${kubernetes_storage_class.consul_home.metadata.0.name}"
  }
}

resource "null_resource" "consul_home_pv" {
  triggers = {
    template_changed = "${data.template_file.consul_home_pv.rendered}"
  }

  provisioner "local-exec" {
    command = "kubectl apply --kubeconfig=${local.config_path} -f - <<EOF\n${data.template_file.consul_home_pv.rendered}\nEOF"
  }

  provisioner "local-exec" {
    when = "destroy"

    command = "kubectl delete --kubeconfig=${local.config_path} persistentvolume consul-home"
  }  
}

resource "kubernetes_storage_class" "consul_home" {
  metadata {
    annotations = "${merge(
      local.common_tags
    )}"
